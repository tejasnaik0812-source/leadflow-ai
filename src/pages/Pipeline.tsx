...
const Pipeline = () => {
  ...
  const handleDrop = (status: LeadStatus) => {
    ...
    if (lead && lead.status !== status) {
      ...
      addActivity({
        id: `a-${Date.now()}`, leadId: draggedLead, leadName: lead.name,
        type: 'status_change',
        description: `Moved from ${LEAD_STATUS_CONFIG[lead.status].label} to ${LEAD_STATUS_CONFIG[status].label}`,
      ...
      <div className="space-y-4 animate-fade-in">
        ...
        <div className="flex gap-3 overflow-x-auto pb-4">
          {PIPELINE_STAGES.map(stage => {
            ...
            return (
              <div key={stage} className="kanban-column min-w-[260px] flex-shrink-0"
                onDragOver={e => e.preventDefault()} onDrop={() => handleDrop(stage)}>
                <div className="flex items-center justify-between mb-3">
                  <Badge variant="secondary" className={`${config.color} text-[10px]`}>{config.label}</Badge>
                  <span className="text-xs font-semibold text-muted-foreground bg-muted rounded-full h-5 w-5 flex items-center justify-center">{stageLeads.length}</span>
                </div>
                <div className="space-y-2">
                  {stageLeads.map(lead => (
                    <div key={lead.id} draggable onDragStart={() => handleDragStart(lead.id)}
                      onClick={() => navigate(`/leads/${lead.id}`)}
                      className={cn('bg-card border border-border/50 rounded-lg p-3 cursor-grab active:cursor-grabbing lead-card-hover', draggedLead === lead.id && 'opacity-50')}>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          <div className="h-1 w-8 bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full" style={{ width: `${lead.score}%` }} /></div>
