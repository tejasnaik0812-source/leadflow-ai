...
const FollowUps = () => {
  ...
  const TaskList = ({ tasks, emptyMsg }: { tasks: typeof mockTasks; emptyMsg: string }) => (
    <div className="space-y-2">
      {tasks.length === 0 ? (
        <p className="text-sm text-muted-foreground py-4 text-center">{emptyMsg}</p>
      ) : (
        tasks.map(task => (
          <div key={task.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors border border-border/30">
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              <Badge variant="secondary" className={cn('text-[10px]', priorityColor(task.priority))}>
                {task.priority}
              </Badge>
